...
function renderTags() {
  ...
  requestAnimationFrame(() => {
    ...
    if (rowH > twoRow + 4) {
      ...
      dom.tagsRow.appendChild(dom.viewMoreTagsBtn);
    } else {
  ...
  let expanded = false;
  dom.viewMoreTagsBtn.addEventListener('click', () => {
    expanded = !expanded;
...
function createVideoCard(id, data) {
  ...
  thumbWrap.addEventListener('click', () => {
    activateEmbed(thumbWrap, playOverlay, id, data.embedUrl);
  });
  ...
  title.addEventListener('click', e => {
    e.preventDefault();
    if (data.targetUrl) window.open(data.targetUrl, '_blank');
  });

  const meta = document.createElement('div');
  meta.className = 'card-meta';
  meta.textContent = `👁 ${formatViews(data.views || 0)} views · Tap to play`;
